...
def subscribe(req:Request,res:Response,userData:CreateUserSchema,connectedDB:Session=Depends(get_db)): # have to be synchronous as move up is synchronous
    ...
    if status == -1:
        return HTTP_RESPONSE(statusCode=409).returnCustomMessage(res,"email already exist")
    return HTTP_RESPONSE(statusCode=200).returnMessage(res) if status == 1 else HTTP_RESPONSE(statusCode=500).returnCustomMessage(res,"something went wrong, please try again")
